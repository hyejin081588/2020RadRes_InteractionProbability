...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
	...
	// replica geometry
	G4VSolid* sol_MyGeom1 = new G4Tubs("Geometry1", 0.0*cm, replicaradius, replicaheight/2, 0.0*deg, 360.0*deg);
	G4LogicalVolume* lv_MyGeom1 = new G4LogicalVolume(sol_MyGeom1, water, "Geometry1");
	new G4PVPlacement(0, G4ThreeVector(0.0*cm, 0.0*cm, -phantomheight/2-replicaheight/2+wantDepth), lv_MyGeom1, "Geometry1", lv_Phantom, false, 200);

	// replica geometry2
	G4VSolid* sol_MyGeom2 = new G4Tubs("Geometry2", 0.0*cm, replicaradius, replicaheight/2, 0.0*deg, 360.0*deg);
	G4LogicalVolume* lv_MyGeom2 = new G4LogicalVolume(sol_MyGeom2, water, "Geometry2");
	new G4PVPlacement(0, G4ThreeVector(0.0*cm, 0.0*cm, -phantomheight/2+replicaheight/2+wantDepth), lv_MyGeom2, "Geometry2", lv_Phantom, false, 200);
	...
	lv_MyGeom1->SetVisAttributes(va_MyGeom);
	lv_MyGeom2->SetVisAttributes(va_MyGeom);
